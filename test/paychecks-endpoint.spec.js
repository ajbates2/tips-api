        ...
        it(`creates a paycheck, responding with 201 and the new paycheck`, function () {
            ...
            return supertest(app)
                ...
                .expect(res => {
                    ...
                    expect(res.body[2].check_total).to.eql(newPaycheck.check_total)
                    expect(res.body[2].user_id).to.eql(newPaycheck.user_id)
                    expect(res.body[2].job.id).to.eql(newPaycheck.job_id)
                    const expectedDate = newPaycheck.date_received.toDateString()
                    const actualDate = new Date(res.body[2].date_received).toDateString()
